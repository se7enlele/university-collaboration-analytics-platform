...
from utils.sqlite_tuning import tune_sqlite_connection
...
def db() -> sqlite3.Connection:
    ...
    tune_sqlite_connection(conn, readonly=True)
...
def data_coverage(university: str | None = None) -> dict:
    if not has_data():
        return {
            "source": "OpenAlex",
            "scope": "public_sample",
    ...
    work_stats = one(
        ...
        SELECT
            COUNT(*) AS works,
            COUNT(CASE WHEN is_international = 1 THEN 1 END) AS international_works,
    ...
    collab_stats = one(
        ...
        SELECT
            ...
            COUNT(DISTINCT collab_institution) AS partner_institutions
        FROM collaborations
        {collab_where}
        """,
        params,
    )
    max_year = work_stats.get("max_year")
    current_year = datetime.now().year
    return {
        "source": "OpenAlex",
        "scope": "public_sample",
        "status": "sample",
        "works": work_stats.get("works") or 0,
        "international_works": work_stats.get("international_works") or 0,
        "collaboration_rows": collab_stats.get("collaboration_rows") or 0,
        "partner_institutions": collab_stats.get("partner_institutions") or 0,
        "min_year": work_stats.get("min_year"),
        "max_year": max_year,
        "requires_validation": True,
        "stale_years": current_year - max_year if max_year else None,
        "validation_note": "公开样例用于发现待复核线索，正式报告建议结合 WoS、学校内部论文库和合作协议台账复核。",
    }
...
def zombie_partners(university: str | None = None) -> dict:
    if not has_data():
        return {"summary": {}, "partners": [], "coverage": data_coverage(university)}
    params = (university,) if university else ()
    university_filter = "AND w.university = ? AND c.university = w.university" if university else ""
    partners = rows(
        f"""
        SELECT
            c.collab_institution AS institution,
    ...
    summary = {
        ...
        "zombie": sum(1 for item in partners if item["status"] == "疑似沉默"),
    }
    zombies = sorted(
        [item for item in partners if item["status"] == "疑似沉默"],
    ...
    return {"summary": summary, "partners": [*zombies, *warnings, *active], "coverage": data_coverage(university)}
