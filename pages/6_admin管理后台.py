...
try:
    ...
    with engine.begin() as conn:
        pending = pd.read_sql(
            text("SELECT id, phone, alipay_trade_no, submitted_at FROM payment_requests WHERE status = 'pending' ORDER BY submitted_at DESC"),
            conn,
        )
...
for _, row in pending.iterrows():
    ...
    if cols[3].button("✅ 通过", key=f"approve_{row['id']}"):
        approve_payment(int(row["id"]), engine)
        st.rerun()
    if cols[4].button("❌ 拒绝", key=f"reject_{row['id']}"):
