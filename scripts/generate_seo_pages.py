...
TOPICS = [
    ...
    },
    {
        ...
        "title": "高校合作数据导入分析",
        "keyword": "高校合作数据导入分析",
        "intent": "让学校把已有 Excel/CSV 合作清单与公开学术数据融合，生成仅本机构可见的分析结果。",
        "h1": "学校已有合作数据，如何变成可分析、可汇报的工作台？",
        "summary": "很多高校已经有协议、出访、项目或论文清单，但数据分散在 Excel 和各部门表格中。AcadMap 支持试点导入用户授权数据，通过字段映射和公开学术数据匹配，形成合作格局、伙伴维护和报告素材。",
        "questions": ["已有合作清单能否匹配到公开论文和机构数据", "内部数据如何只用于本机构分析", "导入后能否生成可汇报的图表和清单"],
        "features": ["Excel/CSV 导入", "字段映射", "OpenAlex 匹配", "数据删除"],
        "cta": "咨询数据导入试点",
    },
]
...
NAV_CSS = """
      ...
      .brand span { display: none; color: #5f6b7a; font-size: 12px; font-weight: 700; }
      .desktop-nav { display: flex; align-items: center; justify-content: center; gap: 10px; min-width: 0; font-size: 14px; }
      ...
      @media (max-width: 1180px) {
        ...
        .desktop-nav > a, .nav-more summary { padding: 0 10px; font-size: 13px; }
      }
    ...
    return f"""
      <nav class="nav" aria-label="主导航">
        ...
        <div class="desktop-nav">
          ...
          <a href="/#home-workflows">解决场景</a>
          <a href="/topics/guoji-hezuo-tijian-baogao.html">数据与方法</a>
          <a href="/pricing">价格试点</a>
        </div>
